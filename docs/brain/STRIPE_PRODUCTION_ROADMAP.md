...
BLOCKED - IMPLEMENTATION/CI. CI passes, but independent review returned material findings.

PR #111 remains open at `9f5780246681b7e8f991677d662170bddec2adf8`, based on `main` at `1c9c6cb783f5838b3b68e2c6327d8e77ce725c82`. Required CI [35312010309](https://github.com/celleree/everywhereposter/actions/runs/35312010309) and repository guard [35312010226](https://github.com/celleree/everywhereposter/actions/runs/35312010226) passed, including the explicit webhook and image-credit regression steps. Fresh independent exact-HEAD review returned FINDINGS; orchestration source verification confirmed both blockers below. S1's exit gate is NOT SATISFIED.

References below are anchored to this PR HEAD and relative to `postiz-app/`:

- HIGH - Lifetime-subscription integrity: `libraries/nestjs-libraries/src/database/prisma/subscriptions/subscription.service.ts:155-183` returns success for disabled billing before organization/current-subscription validation and the lifetime guard. `createOrUpdateSubscription()` then reaches repository upsert, which writes `isLifetime: !!code` (`subscription.repository.ts:138-186`), so an ordinary Stripe event can overwrite a lifetime record. The deletion path also ignores the helper result (`subscription.service.ts:77-85`) and reaches unrestricted customer-scoped `deleteMany` (`subscription.repository.ts:91-99`). Protect existing lifetime records from created/updated/deleted webhook persistence; guarding entitlement side effects alone is insufficient. Preserve valid ordinary non-lifetime bookkeeping.
- MEDIUM - Persisted-tier UI leak: `apps/backend/src/api/routes/users.controller.ts:75-90` still prefers stored subscription tier while billing is disabled. `apps/frontend/src/components/layout/user.context.tsx:33-40` maps it directly to pricing flags used by settings and image-generator consumers. Return the existing billing-disabled effective capability baseline independently of stored tier; do not rewrite subscription data or bypass unrelated role/organization authorization.

The regression in `tests/copy-generation/stripe.billing-activation.spec.ts` mocks the entire repository, uses only non-lifetime records, and incorrectly forbids integrity lookups in disabled mode. Replace those assumptions with tests that exercise the actual repository persistence methods or an isolated database, proving lifetime records remain unchanged and ordinary bookkeeping still works. Cover disabled/unset/invalid and enabled activation plus the `/user/self` to frontend capability mapping.

Next: repair only these two findings and necessary regression coverage on existing PR #111. Preserve signature rejection, disabled integration/user/cron/card-payment safety, and ordinary billing-enabled behavior. No new lifetime product offering or broader lifecycle redesign is implied. Require both focused S1 suites, affected type checks, green final-HEAD CI, and fresh independent exact-HEAD review. No merge is authorized; S2 planning/dispatch remain blocked. Reverify live GitHub before using this checkpoint.
