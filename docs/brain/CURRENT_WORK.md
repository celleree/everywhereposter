...
- S1 explicit billing activation boundary — BLOCKED - IMPLEMENTATION/CI. PR #111 is open at `2dcb49c2f1f9e74b62425d237c438baad87e5b1a`, based on `main` at `1c9c6cb783f5838b3b68e2c6327d8e77ce725c82`. The compilation repair and required CI run [35308563431](https://github.com/celleree/everywhereposter/actions/runs/35308563431) passed, but fresh independent exact-HEAD review found a material webhook bypass: subscription events can still change integration/user/cron entitlements while billing is disabled. Orchestration source verification confirmed the finding. Repair the boundary, explicitly execute focused webhook and image-credit tests, then require green final-HEAD CI and fresh independent review. Reverify live GitHub before using this checkpoint.
...
Stripe S1: repair the billing-disabled webhook entitlement-mutation boundary on existing PR #111. Start from `stripe.controller.ts`, `stripe.service.ts`, and `subscription.service.ts`; preserve signature validation and controlled webhook connectivity while preventing billing-disabled integration/user/cron enforcement. Add and execute focused regression tests, explicitly execute `media.service.image-credits.spec.ts`, and verify required CI on the repaired HEAD before fresh independent exact-HEAD billing review. Keep the repair within S1; do not pull S2 pricing, AI-wallet, BYOK, or unrelated open-PR work into it.
