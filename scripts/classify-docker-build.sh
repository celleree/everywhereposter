...
if ! git diff --no-renames --name-only -z "$BASE_SHA...$HEAD_SHA" > "$changed_files_path"; then
  echo "Unable to determine changed files from the pull-request merge base; defaulting to Docker build."
  write_result true diff-unavailable
  exit 0
fi
